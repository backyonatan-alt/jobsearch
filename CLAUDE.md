...
- **Domain:** **pursuit-playbook.com** (bought Jul 16 2026 on Cloudflare, DNS-only A records → the VM). Replaced the raw-IP `nip.io` URL after the Jul 16 zero-signup read pointed at deliverability/trust; the old `<ip>.nip.io` name stays in nginx + cert so existing sessions keep working.
  - **Live production URL: https://pursuit-playbook.com** (app at `/app`, admin at `/admin`). The deploy host itself is the GitHub Actions secret `DEPLOY_HOST`; if the VM IP ever changes, update the Cloudflare A records, this file, and `web/src/routes/admin/people/+page.svelte`, and re-run certbot.
