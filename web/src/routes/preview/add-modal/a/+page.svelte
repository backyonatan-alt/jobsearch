<script>
  ...
  function clearImage() { attachedImage = null; }
</script>

<svelte:head><title>Option A — Pursuit</title></svelte:head>

<header class="prev-bar">
  <span class="badge">Option A</span>
  <span class="title">Unified zone, quiet hint</span>
  ...
  <div class="modal" role="dialog" aria-modal="true">
    ...
    <div class="paste-block">
      <div class="paste-label">
        ...
        Paste a job posting, URL, screenshot, or recruiter email — we'll fill the fields below.
      </div>
      ...
      >
        {#if attachedImage}
          <div class="attached">
            ...
            <button class="x" onclick={clearImage} aria-label="Remove">×</button>
          </div>
        {:else}
          <textarea
            bind:value={text}
            onpaste={onPaste}
            placeholder="Drop a screenshot here, or paste any text / URL"
            rows="4"
          ></textarea>
        {/if}
      </div>
      ...
      <div class="paste-row">
        ...
        <span class="tip">
          <span class="muted">Tip:</span> on LinkedIn, <kbd>⌘A</kbd> then <kbd>⌘V</kbd> here — or <kbd>⌘⇧4</kbd> to screenshot, then paste.
        </span>
      </div>
    </div>
    ...
    <div class="fields">
      ...
      <label>Status
        <select>
          <option>Applied</option><option>Wishlist</option><option>Screen</option><option>Interview</option><option>Offer</option><option>Rejected</option><option>Withdrawn</option>
        </select>
      </label>
...
<style>
  ...
  .prev-bar {
    ...
    padding: 10px 16px; background: var(--ink); color: white; font-size: 12px;
  }
  ...
  .drop { position: relative; border-radius: 8px; transition: outline-color 120ms ease; outline: 0 dashed transparent; outline-offset: -1px; }
  .drop.drag { outline: 2px dashed var(--accent); background: var(--accent-tint); }
  .drop textarea {
    width: 100%; font: inherit; font-family: var(--sans); font-size: 13.5px;
    color: var(--ink); background: var(--surface);
    border: 1px solid var(--rule); border-radius: 8px;
    padding: 12px; outline: none; resize: vertical; min-height: 88px; line-height: 1.5;
  }
  ...
  .attached .x { margin-left: auto; background: transparent; border: 0; color: var(--mute); font-size: 18px; cursor: pointer; line-height: 1; }
  .attached .x:hover { color: var(--ink); }

  .paste-row { display: flex; align-items: center; gap: 12px; }
  .tip { font-size: 12px; color: var(--mute); }
  .tip .muted { color: var(--mute-2); }
  .tip kbd {
